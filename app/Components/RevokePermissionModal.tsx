...
export default function RevokePermissionModal({ userId, user }: any) {
  ...
  const [RevokeUserPermission] = useRevokeRoleWithPermissionsMutation();
  ...
  const handlePermissionToggle = (permissionName) => {
    setForm((prevForm) => {
      ...
      const isPermissionSelected = prevForm.permissions.includes(permissionName);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setIsOpen(false);
      toast({ title: "Revoked User Roles and Permission Successfully!" });
    } catch (error) {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[425px] md:max-w-[1000px]">
        <DialogHeader className="flex flex-col justify-center items-center">
          <DialogTitle>Revoke Roles and Permissions</DialogTitle>
          <DialogDescription>
